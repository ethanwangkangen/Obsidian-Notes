...
- `ipconfig`
	- Shows settings of network interfacce
	- `/all` 
- `arp -a`
	- Recall: ARP translates IP addresses to physical addresses
	- `-a`: diasplay host's curent ARP entries
- `nslookup`
	- DNS lookup
- `netstat`
	- Display current active connections to host
	- `-an`: print connections and listening ports, place in numerical form
