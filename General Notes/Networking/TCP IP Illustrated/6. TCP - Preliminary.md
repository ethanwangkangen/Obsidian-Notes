...
# Windows of packets
- Sender has a window
	- How many bytes the sender is currently allowed to have in flight
- Rationale
	- Don't want to overload receiver
...
# Congestion control
- Sender slows ITSELF down to **not overwhelm network** between itself and receiver
...
> For reliable UDP project: throw out **both**. Reason: flow control don't need as SPSC ring buffer between threads is backpressure mechanism. Congestion control don't want as we want it to be fast
...
- **Options** (variable, 0–40 bytes) — the important ones are:
	...
	- **WSCALE** (Window scale) — multiplier for the window size field, negotiated at handshake, lets you have windows much larger than 64KB
